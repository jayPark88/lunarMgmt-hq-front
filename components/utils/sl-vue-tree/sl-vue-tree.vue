<template>
        ...
        >
          ...
          >
            ...
            <div class="sl-vue-tree-title">
              <span
                v-if="!node.isLeaf && node.children.length != 0"
              ...
              <span v-else class="sl-vue-tree-no-toggle"></span>
  ...
  export default {
    ...
    props: {
      ...
      multiselectKey: {
        ...
        validator(value) {
          ...
          multiselectKeys = multiselectKeys.filter((keyName) =>
            allowedKeys.includes(keyName),
          )
    ...
    methods: {
      ...
      onExternalDragoverHandler(_node, event) {
      ...
      onExternalDropHandler(_node, event) {
        const root = this.getRoot()
        const cursorPosition = root.getCursorPositionFromCoords(
          event.clientX,
          event.clientY,
      ...
      remove(paths) {
        ...
        const newNodes = this.copy(this.currentValue)
        this.traverse((node, nodeModel, _siblings) => {
